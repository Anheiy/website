...
function Window({ id, title, x, y, visible, content, onToggle, width, height, z =1, bgColor, icon_path = "./itchio.png", pos = "absolute"}) {
    ...
    return (
        <div id={id} ref={setNodeRef} style={style} className={`w-window h-window rounded-md ${bgColor} touch-none shadow-slate-800 shadow-2xl border-blue-700 border-4 overflow-hidden`}>
            <div className="flex items-center bg-blue-700 w-window h-8 shadow-sm shadow-slate-500">
                ...
                <div className={`flex-1 flex items-center ${isDragging ? "cursor-grabbing" : "cursor-grab"} select-none`}{...listeners}{...attributes}>
                    <img src={icon_path} className="size-6 ml-2" />
                    <div className="text-white font-Jersey10 ml-2 text-xl">{title}</div>
                </div>
                <button icon_path={icon_path} onClick={onToggle}className="mr-2 size-5 text-blue-100 font-Jersey10 flex items-center justify-center hover:text-red-500">
                   [X]
                </button>
            </div>
